...
def collect_tradition_data(options, auth=None):
    '''Iterate through the given tradition, saving an SVG and a list of
    readings for each section into the necessary output directory.'''
...
def collect_section_data(options, section, outdir, auth=None):
    ...
    # Collect the section readings
    r = requests.get("%s/readings" % baseurl, auth=auth)
    r.raise_for_status()
    with open('%s/readings.json' % sectiondir, 'w', encoding='utf-8') as rdglist:
        rdglist.write(r.text)
